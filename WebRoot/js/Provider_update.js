...
$(function() {
	...
	$("#submit").click(function(){
		...
		var from=$("#proUpdateFrom").serialize();
		if(num==1){
			$.post("${pageContext.request.contextPath}/smbms/ProviderServlet/update",from,function(Return){
				if(Return=="1"){
				alert("修改成功");
				window.location.href = "${pageContext.request.contextPath}/smbms/ProviderServlet/Provider";}
				else{
					alert("修改失败");
					window.location.href = "${pageContext.request.contextPath}/smbms/ProviderServlet/IdUpdate";
				}
			});
		}else{
			alert("请将资料补全");
